"""What Weight promises: the length of the (50 - H1-A3, 4 x loudness) vector.
...
The loudness leg is scaled by ``LOUDNESS_WEIGHT``, which is tuned by ear and has
been retuned more than once. Every case here is written in terms of it rather
than of the number it currently holds, so retuning it moves the inputs and
leaves the shape of the maths -- which is what these tests are about -- alone.
"""
...
from signal_processing.Weight import H1_A3_REFERENCE, LOUDNESS_WEIGHT, calculate_weight

#: The loudness that contributes a leg of 4, whatever the coefficient is.
LEG_OF_4 = 4.0 / LOUDNESS_WEIGHT


@pytest.mark.parametrize("h1_a3, loudness, expected", [
    (H1_A3_REFERENCE, 0.0, 0.0),                  # at the reference, silent: no weight
    (H1_A3_REFERENCE, 3.0, 3.0 * LOUDNESS_WEIGHT),  # at the reference: loudness alone
    (H1_A3_REFERENCE - 4.0, 0.0, 4.0),            # silent: the H1-A3 distance alone
    (47.0, LEG_OF_4, 5.0),                        # both: a 3-4-5 triangle
    (H1_A3_REFERENCE + 3.0, LEG_OF_4, 5.0),       # above the reference is still a distance
])
...
def test_timepoints_are_passed_through_untouched():
    ...
    result = calculate_weight(t, np.array([50.0, 47.0, 40.0]),
                              np.array([0.0, LEG_OF_4, 0.0]))
...
def test_a_rejected_frame_does_not_take_its_neighbours_with_it():
    result = calculate_weight(np.arange(3.0),
                              ...
                              np.array([LEG_OF_4, LEG_OF_4, LEG_OF_4]))
